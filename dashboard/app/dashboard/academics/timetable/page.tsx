...
function slotKey(weekday: number, period: number) { return `${weekday}_${period}`; }
...
export default function TimetablePage() {
  ...
  // periods[i] = {period: i+1, start, end}
  const [periods, setPeriods] = useState(DEFAULT_PERIODS);

  // grid[weekday][period] = SlotData | null
  ...
  useEffect(() => {
    Promise.all([
      ...
      apiFetch<Subject[]>("/api/academics/admin/subjects/?active_only=true"),
      ...
      <div style={{ padding: "24px 28px 48px" }}>
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div>
            ...
            <h1 style={{ fontSize: 30, fontWeight: 700, letterSpacing: "-0.02em", color: "var(--ink)", lineHeight: 1 }}>Timetable</h1>
          </div>
          {timetable && (
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              <button onClick={save} disabled={saving}
                style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--primary)", color: "#fff", border: "none", borderRadius: 10, padding: "11px 20px", fontWeight: 700, fontSize: 14, cursor: "pointer", opacity: saving ? 0.6 : 1 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-soft)" }}>Classroom</label>
            <select value={selectedClass} onChange={(e) => setSelectedClass(Number(e.target.value) || "")}
              style={{ background: "var(--surface)", border: "1px solid var(--stroke)", borderRadius: 9, padding: "9px 12px", fontSize: 13, color: "var(--ink)", outline: "none", minWidth: 180 }}>
        ...
        {!selectedClass ? (
          ...
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            ...
            <p style={{ fontSize: 16, fontWeight: 700, color: "var(--ink)", marginBottom: 6 }}>
              No timetable for {cls?.name ?? "this class"}
            </p>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 11, color: "var(--ink-dim)", marginLeft: "auto", alignSelf: "center" }}>
                Click any cell to edit · Click subject/teacher dropdowns to assign
              </span>
            </div>
              ...
              <div style={{ minWidth: 720 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: `90px repeat(${numDays}, 1fr)`, gap: 6, marginBottom: 6 }}>
                  ...
                  {DAYS.slice(0, numDays).map((d, i) => (
                    <div key={d} style={{ textAlign: "center", padding: "8px 0", background: "var(--surface-raised)", borderRadius: 8, fontSize: 12, fontWeight: 700, color: "var(--ink-soft)" }}>
                      {d}
                    </div>
                  ))}
                </div>
                  ...
                  <div key={p.period} style={{ display: "grid", gridTemplateColumns: `90px repeat(${numDays}, 1fr)`, gap: 6, marginBottom: 6 }}>
                    ...
                    <PeriodHeader
                      ...
                      onTimeChange={(start, end) => {
                        setPeriods(prev => prev.map((x, i) => i === pi ? { ...x, start, end } : x));
                      }}
                      onDelete={() => {
                    ...
                    {Array.from({ length: numDays }, (_, i) => i + 1).map(day => (
                      <SlotCell
                        key={day}
                        slot={grid[day]?.[p.period] ?? null}
                        subjects={subjects}
                        teachers={teachers}
                        onChange={(update) => setCellSlot(day, p.period, update)}
                      />
                    ))}
                  </div>
                ))}
