...
export function useTestWebSocket({ sessionId, onReady }) {
    ...
    const connect = useCallback(() => {
        ...
        socket.onmessage = (evt) => {
            ...
            if (type === "timer_tick") {
                tickTimer(data.time_remaining);
                return;
            }
            if (type === "test_complete") {
                setCompleted(data.reason, data.score, data.max_score, data.percentage, data.grade);
                return;
            }
            if (type === "answer_ack") {
            ...
            if (type === "transcript") {
                ...
                window.dispatchEvent(new CustomEvent("exam:transcript", {
    ...
    const stopRecording = useCallback((questionId) => {
