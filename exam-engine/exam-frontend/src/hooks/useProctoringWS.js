...
const FRAME_INTERVAL_MS = 5000; // capture every 5 seconds
export function useProctoringWS(sessionId, enabled) {
    ...
    const captureAndSend = useCallback(() => {
        ...
        if (!video || !video.readyState || ws.current?.readyState !== WebSocket.OPEN)
            return;
        if (!canvas.current) {
        ...
        ws.current.send(JSON.stringify({ base64, camera_width: 320, camera_height: 240 }));
    }, []);
    useEffect(() => {
        if (!enabled)
            return;
            ...
            ws.current.onmessage = (evt) => {
                try {
                    ...
                    if (severity === "critical") {
                        if (people_count > 1)
                            setProctoringAlert("multiple_people");
                        else if (communication_device)
                            setProctoringAlert("phone_detected");
                    }
